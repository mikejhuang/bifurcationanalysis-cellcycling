...
[t,result]=ode45(@G1S_Func,tspan,initial,options,pars);

figure(1); %Plots for pRB and E2F
plot(t,result(:,1),'r',t,result(:,2),'g',t,result(:,4),'b',t,result(:,9),'c','LineWidth',1.5,'LineSmoothing','on');
xlabel('Time');ylabel('Concentration');title('Time course of computed protein concentrations.')
legend('pRB','E2F','Cyclin D/cdk4,6','Cyclin E/cdk2'); grid on;

figure(2); %Plots for cyclin D & E, activated cyclin D &E
plot(t,result(:,3),'b',t,result(:,8),'g','LineWidth',1.5,'LineSmoothing','on')
xlabel('Time');ylabel('Concentration');title('Bistability of Cyclins');
legend('CycD','CycE'); grid on;

figure(3) %plots for the generation rate of cyclin E
plot(t,0.06*result(:,2).*0.6*7./((0.6+result(:,1)).*(7+result(:,6))),'LineWidth',1.5,'LineSmoothing','on')
