...
for i=1:20
%     if i>1
%         initial = result{i-1}(length(result{i-1}(:,1)),:);
%     end
    pars(40)=i*0.0009;
    options = odeset('reltol',1e-6);
    [t,result{i}]=ode45(@G1S_Func,tspan,initial,options,pars);
    subplot(4,5,i);plot(t,result{i});axis([0 tlen 0 15]);
    title(pars(40));
end
...
plot(Fm,Stability_Low,'g','linesmoothing','on');
hold on
plot(Fm(4:16),Stability_High(4:16),'g','linesmoothing','on');grid on;
xlabel('F_m')
ylabel('E2F-1')
